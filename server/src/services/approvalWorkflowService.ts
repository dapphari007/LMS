...
export const getApprovalWorkflowForDuration = async (
  ...
  try {
    ...
    // Log the requester role information for debugging
    if (approvalWorkflow && approvalWorkflow.requesterRole) {
      logger.info(`Found workflow with requester role: ${approvalWorkflow.requesterRole.name}`);
    }
